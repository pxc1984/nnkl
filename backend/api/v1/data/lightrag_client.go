...
func (c *LightRAGClient) SendText(ctx context.Context, text, fileSource string) error {
	...
	if err != nil { return fmt.Errorf("marshal lightrag request: %w", err) }
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, c.baseURL+"/documents/text", bytes.NewReader(body))
	if err != nil { return fmt.Errorf("create lightrag request: %w", err) }
	req.Header.Set("Content-Type", "application/json")
	c.setAPIKey(req)
	resp, err := c.client.Do(req)
	if err != nil { return fmt.Errorf("lightrag request failed: %w", err) }
...
func (c *LightRAGClient) Query(ctx context.Context, query, mode string) (*LightRAGQueryResponse, error) {
	...
	if err != nil { return nil, fmt.Errorf("marshal lightrag query request: %w", err) }
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, c.baseURL+"/query", bytes.NewReader(body))
	if err != nil { return nil, fmt.Errorf("create lightrag query request: %w", err) }
	req.Header.Set("Content-Type", "application/json")
	c.setAPIKey(req)
	resp, err := c.client.Do(req)
	if err != nil { return nil, fmt.Errorf("send lightrag query request: %w", err) }
	defer resp.Body.Close()
	...
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil { return nil, fmt.Errorf("decode lightrag response: %w", err) }
	return &result, nil
}
